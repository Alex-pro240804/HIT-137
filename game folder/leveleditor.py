...
# Flags for controlling single level changes
level_change_up = False
level_change_down = False
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP and not level_change_up:
                level += 1
                level_change_up = True
                world_data = [[-1] * MAX_COLS for _ in range(VISIBLE_ROWS)]
                for tile in range(0, MAX_COLS):
                 world_data[VISIBLE_ROWS - 1][tile] = 0 
            if event.key == pygame.K_DOWN and level > 0:
                level -= 1
                level_change_down = True
                world_data = [[-1] * MAX_COLS for _ in range(VISIBLE_ROWS)]
                for tile in range(0, MAX_COLS):
                 world_data[VISIBLE_ROWS - 1][tile] = 0
